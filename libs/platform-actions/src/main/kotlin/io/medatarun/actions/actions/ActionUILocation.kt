package io.medatarun.actions.actions

object ActionUILocation {
    const val auth_actor="auth_actor"
    const val auth_actor_role="auth_actor_role"
    const val auth_actor_roles="auth_actor_roles"
    const val auth_actors="auth_actors"
    const val auth_role_permission="auth_role.permission"
    const val auth_role_permissions="auth_role.permissions"
    const val auth_role="auth_role"
    const val auth_roles="auth_roles"
    const val entity="entity"
    const val entity_hidden="entity.hidden_detail"
    const val entity_attribute="entity.attribute"
    const val entity_attribute_hidden="entity.attribute.hidden_detail"
    const val entity_attributes="entity.attributes"
    const val entity_relationships="entity.relationships"
    const val global="global"
    const val hidden="hidden"
    const val model_entities="model.entities"
    const val model_hidden="model.hidden_detail"
    const val model_overview="model.overview"
    const val model_relationships="model.relationships"
    const val model_types="model.types"
    const val models="models"
    const val preferences="preferences"
    const val relationship="relationship"
    const val relationship_hidden="relationship.hidden_detail"
    const val relationship_attribute="relationship.attribute"
    const val relationship_attribute_hidden="relationship.attribute.hidden_detail"
    const val relationship_attributes="relationship.attributes"
    const val relationship_role="relationship.role"
    const val relationship_roles="relationship.roles"
    const val type="type"
    const val type_hidden="type.hidden_detail"
}
