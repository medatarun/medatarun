...
class TagsCoreExtension : MedatarunExtension {
    ...
    override fun initServices(ctx: MedatarunServiceCtx) {
        ...
        eventSystem.registerObserver(TagLocalScopeBeforeDeleteEvent::class) { evt ->
            tagCmds.dispatch(
                TagCmdEnveloppe(
                    traceabilityRecord = evt.traceabilityRecord,
                    cmd = TagCmd.TagLocalScopeDelete(evt.tagScopeRef)
                )
            )
        }
    ...
    override fun initContributions(ctx: MedatarunExtensionCtx) {
        ...
        ctx.registerContribution(SecurityPermissionsProvider::class, TagSecurityPermissionsProvider())
