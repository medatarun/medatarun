...
class FrictionlessConverter {
    ...
    fun convert(
        ...
        val finalName = modelName?.trimToNull()?.let { TextSingleLine(it) } ?: model.name
    ...
    private fun readAsDataPackage(
        ...
        val model = ModelAggregateInMemory(
            model = ModelInMemory(
                ...
                name = datapackage.title?.let { TextSingleLine(it) },
                description = datapackage.description?.let { TextMarkdown(it) },
                version = try {
    ...
    private fun toEntity(
        ...
        val attributes = schema.fields.map { field ->
            AttributeInMemory(
                ...
                name = field.title?.let(::TextSingleLine),
                description = field.description?.let(::TextMarkdown),
        ...
        val entity = EntityInMemory(
            ...
            name = entityName?.let(::TextSingleLine),
            description = entityDescription?.let(::TextMarkdown),
