...
class Model_Delete_Test {
    ...
    fun `delete model removes it from storage`() {
        ...
        assertNull(query.findModelAggregateOptional(modelRef("recipe")))
        ...
        assertNotNull(query.findModelAggregateOptional(modelRef("movie")))
        assertNotNull(query.findModelAggregateOptional(modelRef("vehicle")))
        assertNotNull(query.findModelAggregateOptional(modelRef("videogame")))

        env.dispatch(ModelAction.Model_Delete(modelRefKey("vehicle")))
        assertNull(query.findModelAggregateOptional(modelRef("vehicle")))
        assertNull(env.tagQueries.findTagByIdOptional(recipeUiSearchTag.id))
        ...
        assertNull(query.findModelAggregateOptional(modelRef("movie")))
        assertNull(env.tagQueries.findTagByIdOptional(recipeUiSearchTag.id))
        assertNull(env.tagQueries.findTagByIdOptional(vehicleImportedTag.id))
        ...
        env.replayWithRebuild {
            assertNull(query.findModelAggregateOptional(modelRef("videogame")))
            assertNull(env.tagQueries.findTagByIdOptional(recipeUiSearchTag.id))
            assertNull(env.tagQueries.findTagByIdOptional(vehicleImportedTag.id))
            assertNull(env.tagQueries.findTagByIdOptional(movieUiResultTag.id))
