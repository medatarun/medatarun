...
export function useTags(scope?: TagScopeRef): {
  ...
  const globalTags = useTagSearch(
    tagSearchByScope(globalTagScope),
    globalTagsEnabled,
  );
  const scopedTags = useTagSearch(
    tagSearchByScope(scope ?? globalTagScope),
    scopedTagsEnabled,
  );
  ...
  const tags = useMemo(() => {
    const tagDtos =
      scope === undefined ? allTagDtos : [...globalTagDtos, ...scopedTagDtos];
  ...
  return {
    tags: tags,
    isPending:
      (allTagsEnabled && allTags.isPending) ||
      (globalTagsEnabled && globalTags.isPending) ||
      (scopedTagsEnabled && scopedTags.isPending) ||
      tagGroups.isPending,
    error:
      allTags.error ?? globalTags.error ?? scopedTags.error ?? tagGroups.error,
  };
}
