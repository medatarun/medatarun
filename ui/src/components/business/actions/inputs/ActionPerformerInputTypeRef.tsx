...
import {ErrorBox, InputSelect, type InputSelectOption, Loader} from "@seij/common-ui";
import type {ActionPerformerInputProps} from "./ActionPerformerInputProps.tsx";
import {toProblem} from "@seij/common-types";
import {adaptPropsRefIdToRawId, normalizeValueStringOrEmpty} from "./ActionPerformerInput.utils.ts";
...
export function ActionPerformerInputTypeRef(props: ActionPerformerInputProps) {
  const modelId = props.request.displayedSubject.kind === "resource" ? (props.request.displayedSubject.refs["modelId"] ?? null) : null
  if (!modelId) return <ErrorBox
    error={toProblem("Can not create input for type. Can not find modelId on which operate in action performer request")}/>
  const wrappedProps = adaptPropsRefIdToRawId(props)
  return <ActionPerformerInputTypeIdSafe {...wrappedProps} modelId={modelId}/>
}

function ActionPerformerInputTypeIdSafe({modelId, value, onValueChange, disabled}: ActionPerformerInputProps<string> & {
  modelId: string
}) {
  ...
  return <InputSelect value={valueSafe} options={options} onValueChange={onValueChange} disabled={disabled}/>
}
